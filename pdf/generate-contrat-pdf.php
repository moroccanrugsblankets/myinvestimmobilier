...
function replaceContratTemplateVariables($template, $contrat, $locataires) {
    ...
    $vars = [
        ...
        '{{dpe_classe}}' => htmlspecialchars($contrat['dpe_classe'] ?? ''),
        '{{ges_classe}}' => htmlspecialchars($contrat['dpe_ges'] ?? ''),
        '{{dpe_numero}}' => htmlspecialchars($contrat['dpe_numero'] ?? ''),
        '{{dpe_validite}}' => htmlspecialchars($contrat['dpe_valable_jusqu_a'] ?? ''),
    ];
...
function buildSignaturesTable($contrat, $locataires) {
    ...
    $html = '<table cellspacing="0" cellpadding="10" border="0" style="width: 100%; margin-top: 20px; background: transparent;border: none;">';
    $html .= '<tbody><tr>';
